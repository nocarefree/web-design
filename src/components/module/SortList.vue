...
const props = defineProps({
	...
	blocks: { type: Array as PropType<ModuleBlock[]> },
})
...
onMounted(() => {
    ...
    draggable.on('drag:stop',function(event: any){
        ...
        let nIndex = sortIds.findIndex(i=>i==source);
        let oIndex = nodes.findIndex(i=>i==source);
        let last = 0;
        ...
        if(oIndex != nIndex){
            if(nIndex < last){
                parentNode.insertBefore(source, nodes[nIndex])
            }else{
                parentNode.insertBefore(source, parentNode.children[parentNode.children.length-1])
            }
        }
